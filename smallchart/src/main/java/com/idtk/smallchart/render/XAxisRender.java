...
public class XAxisRender extends AxisRender {
    ...
    public XAxisRender(IXAxisData xAxisData,IYAxisData yAxisData) {
        super();
    ...
    public void drawGraph(Canvas canvas) {
        ...
        for (int i=0;(xAxisData.getInterval()*i+xAxisData.getMinimum())<=xAxisData.getMaximum();i++){
            ...
            mPoint.x = TextPathX;
            mPoint.y = -TextPathY;
            textCenter(new String[]{numberFormat.format(xAxisData.getInterval()*i+xAxisData.getMinimum())+xAxisData.getUnit()},
                    mPaint,canvas, mPoint, Paint.Align.CENTER);
            canvas.drawLine(TextPathX,0,TextPathX,-yAxisData.getAxisLength(),linePaint);
            canvas.restore();
        }
        /**
         * 箭头
