...
public class Compute {
    ...
    protected void initMaxMin(float max, float min, int count, IAxisData axisData){
        ...
        Log.i("Compute","max="+max+",min="+min+",axisData.getMinimum()="+axisData.getMinimum()+",axisData.getMaximum()="+axisData.getMaximum());
        //装最大最小值
        if (count == 0){
...
//            LogUtil.d("TAG",max+":"+min+":"+scaling+":"+length);
        ...
        Log.i("Compute","max="+max+",min="+min+",scaling="+scaling);
