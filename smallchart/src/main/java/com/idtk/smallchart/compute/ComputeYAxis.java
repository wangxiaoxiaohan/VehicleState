...
public class ComputeYAxis<T extends IBarLineCurveData> extends Compute {
    ...
    public void computeYAxis(ArrayList<T> mBarLineCurveDatas){
        for (int i=0; i<mBarLineCurveDatas.size();i++){
            ...
            for (int j=0; j<mBarLineCurveData.getValue().size(); j++){
                Log.i("ComputeYAxis","computeYAxis:"+(mBarLineCurveData.getValue().get(j).x)+" : "+(mBarLineCurveData.getValue().get(j).y)+":i="+i+",j="+j);
                 totalY+=mBarLineCurveData.getValue().get(j).y;
            }
            Log.i("ComputeYAxis","computeYAxis AverageValue="+(totalY/mBarLineCurveData.getValue().size()));

            yAxisData.setAverageValue(totalY/mBarLineCurveData.getValue().size());
        }
    ...
    public void computeYAxisMin(ArrayList<T> mBarLineCurveDatas){
        for (int i=0; i<mBarLineCurveDatas.size();i++){
            ...
            for (int j=0; j<mBarLineCurveData.getValue().size(); j++){
                Log.i("ComputeYAxis","computeYAxisMin:"+(mBarLineCurveData.getValue().get(j).x)+":"+(mBarLineCurveData.getValue().get(j).y)+":i="+i+",j="+j);
            }
        }
        //默认所有的BarLineCurveData。getValue()长度相同
        if (mBarLineCurveDatas.size()>0)
